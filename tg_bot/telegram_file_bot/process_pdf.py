...
from unstructured.partition.pdf import partition_pdf  # версия unstructured 0.11.5
...
languages = ["eng"]  # пример: ["eng", "por"] (по умолчанию "eng")

## model_name
# @requires_dependencies("unstructured_inference")
# yolox: best model for table extraction. Other options are yolox_quantized, detectron2_onnx и chipper в зависимости от расположения файла
    ...
    for entry in data:
        ...
        if page_number and text:
            if page_number in page_texts:
                page_texts[page_number] += " " + text  # Конкатенируем текст, если страница уже существует
            else:
                page_texts[page_number] = text
    return page_texts


def _save_json(data, filename, indent, encoding):
...
def process_and_save(file_path: str) -> str:
    """
    Обрабатывает PDF файл и сохраняет результаты в JSON.

    :param file_path: Путь к PDF файлу.
    :return: Путь к обработанному JSON файлу.
    """
    # Возвращает Iterable[Element], присутствующие на страницах разобранного PDF документа
    ...
    json_output = _elements_to_json(elements, filename=file_path)  # Может занять время для больших файлов
